...
namespace BTL.Controllers
{
    public class SubscriptionController : Controller
    {
        ...
        public SubscriptionController(BTLContext context, INotyfService notyfService)
        {
            _context = context;
        ...
        {
            int companyId = int.Parse(User.FindFirst("CompanyId").Value); // Lấy CompanyId từ Claims của người dùng

            if (IsCompanySubscribedToService(companyId, serviceName))
            {
                // Công ty đã đăng ký dịch vụ, chuyển hướng đến tính năng cụ thể
                return Redirect(returnUrl);
            }
            else
            {
                // Công ty chưa đăng ký dịch vụ, chuyển hướng đến trang đăng ký dịch vụ
                return RedirectToAction("SubscriptionNotice", new { serviceName });
            }
        }
